 ...
 */
export function createEOACode7702Tx(txData, opts = {}) {
 ...
 */
export function createEOACode7702TxFromBytesArray(values, opts = {}) {
    if (values.length !== 10 && values.length !== 13) {
        throw EthereumJSErrorWithoutCode('Invalid EIP-7702 transaction. Only expecting 10 values (for unsigned tx) or 13 values (for signed tx).');
    }
 ...
 */
export function createEOACode7702TxFromRLP(serialized, opts = {}) {
    if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.EOACodeEIP7702)) === false) {
        throw EthereumJSErrorWithoutCode(`Invalid serialized tx input: not an EIP-7702 transaction (wrong tx type, expected: ${TransactionType.EOACodeEIP7702}, received: ${bytesToHex(serialized.subarray(0, 1))}`);
    }
