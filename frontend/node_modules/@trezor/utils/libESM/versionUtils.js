...
export const isVersionArray = arr => Array.isArray(arr) && arr.length === 3 && arr.every(number => typeof number === 'number' && number >= 0);
export const tryParse = version => version.match(/^(\d+)\.(\d+)\.(\d+)([+-].*)?$/)?.slice(1, 4).map(n => Number(n));
const validateArray = version => isVersionArray(version) ? version : null;
const ensureArray = version => (typeof version === 'string' ? tryParse(version) : validateArray(version)) ?? throwError(`version string is in wrong format: ${version}`);
