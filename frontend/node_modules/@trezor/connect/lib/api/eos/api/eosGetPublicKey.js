...
class EosGetPublicKey extends AbstractMethod_1.AbstractMethod {
  ...
  get confirmation() {
    return {
      view: 'export-address',
      label: this.params.length > 1 ? 'Export multiple Eos public keys' : `Export Eos public key for account #${(0, pathUtils_1.fromHardened)(this.params[0].address_n[2]) + 1}`
    };
  }
  async run() {
    const responses = [];
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const {
        message
      } = await cmd.typedCall('EosGetPublicKey', 'EosPublicKey', batch);
      responses.push({
