...
class CardanoSignTransaction extends AbstractMethod_1.AbstractMethod {
  init() {
    ...
    if (payload.requiredSigners) {
      requiredSigners = payload.requiredSigners.map(requiredSigner => ({
        key_path: requiredSigner.keyPath ? (0, pathUtils_1.validatePath)(requiredSigner.keyPath, 3) : undefined,
        key_hash: requiredSigner.keyHash
      }));
    }
    const collateralReturnWithData = payload.collateralReturn ? (0, cardanoOutputs_1.transformOutput)(payload.collateralReturn) : undefined;
    let referenceInputs = [];
    ...
    this.params = {
      ...
      derivationType: typeof payload.derivationType !== 'undefined' ? payload.derivationType : constants_1.PROTO.CardanoDerivationType.ICARUS_TREZOR,
  ...
  _ensureFirmwareSupportsParams() {
    ...
    params.certificatesWithPoolOwnersAndRelays.forEach(({
      ...
      if (certificate.type === constants_1.PROTO.CardanoCertificateType.STAKE_REGISTRATION_CONWAY || certificate.type === constants_1.PROTO.CardanoCertificateType.STAKE_DEREGISTRATION_CONWAY || certificate.type === constants_1.PROTO.CardanoCertificateType.VOTE_DELEGATION) {
  ...
  async run() {
    ...
    const serializedTx = coin_selection_1.trezorUtils.signTransaction(unsignedTx.body, witnesses, {
      testnet
    });
    return {
      ...result,
