...
class CardanoGetNativeScriptHash extends AbstractMethod_1.AbstractMethod {
  init() {
    ...
    this.params = {
      ...
      derivation_type: typeof payload.derivationType !== 'undefined' ? payload.derivationType : constants_1.PROTO.CardanoDerivationType.ICARUS_TREZOR
    };
  }
  get info() {
  ...
  async run() {
    const {
      message
    } = await this.device.getCommands().typedCall('CardanoGetNativeScriptHash', 'CardanoNativeScriptHash', {
