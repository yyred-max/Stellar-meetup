...
class CardanoGetPublicKey extends AbstractMethod_1.AbstractMethod {
  ...
  init() {
    ...
    this.params = payload.bundle.map(batch => {
      ...
      return {
        address_n: path,
        derivation_type: typeof batch.derivationType !== 'undefined' ? batch.derivationType : constants_1.PROTO.CardanoDerivationType.ICARUS_TREZOR,
  ...
  get confirmation() {
    return {
      view: 'export-xpub',
      label: this.params.length > 1 ? 'Export multiple Cardano public keys' : `Export Cardano public key for account #${(0, pathUtils_1.fromHardened)(this.params[0].address_n[2]) + 1}`
    };
  }
  async run() {
    const responses = [];
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const {
        message
      } = await cmd.typedCall('CardanoGetPublicKey', 'CardanoPublicKey', batch);
      responses.push({
