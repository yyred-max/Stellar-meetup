...
class SolanaComposeTransaction extends AbstractMethod_1.AbstractMethod {
  ...
  async run() {
    ...
    const [recipientAccountOwner, recipientTokenAccounts] = this.params.token ? await (0, solanaUtils_1.fetchAccountOwnerAndTokenInfoForAddress)(backend, this.params.toAddress, this.params.token.mint, this.params.token.program) : [undefined, undefined];
    ...
    if (this.params.token && !tokenTransferTxAndDestinationAddress) throw constants_1.ERRORS.TypedError('Method_InvalidParameter', 'Token accounts not found');
    const tx = tokenTransferTxAndDestinationAddress ? tokenTransferTxAndDestinationAddress.transaction : await (0, solanaUtils_1.buildTransferTransaction)(this.params.fromAddress, this.params.toAddress, this.params.amount, this.params.blockHash, this.params.lastValidBlockHeight, this.params.priorityFees || solanaUtils_1.dummyPriorityFeesForFeeEstimation);
    const isCreatingAccount = this.params.token && recipientTokenAccounts === undefined && (recipientAccountOwner === solana_1.SYSTEM_PROGRAM_PUBLIC_KEY || recipientAccountOwner == null);
