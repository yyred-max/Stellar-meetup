...
class SolanaSignTransaction extends AbstractMethod_1.AbstractMethod {
  ...
  payloadToPrecomposed() {
    try {
      ...
      for (const instruction of message.instructions) {
        if (instruction.programAddress === compute_budget_1.COMPUTE_BUDGET_PROGRAM_ADDRESS && instruction.data) {
          const data = instruction.data;
          const type = (0, compute_budget_1.identifyComputeBudgetInstruction)({
            data
          });
        ...
        if (instruction.programAddress === system_1.SYSTEM_PROGRAM_ADDRESS && instruction.data && instruction.accounts) {
        ...
        if (instruction.programAddress === token_1.TOKEN_PROGRAM_ADDRESS && instruction.data && instruction.accounts) {
          const instructionSafe = {
            ...instruction,
            data: instruction.data,
            accounts: instruction.accounts
          ...
          if (type === token_1.TokenInstruction.TransferChecked) {
            ...
            const tokenInfo = this.payload.additionalInfo?.tokenAccountsInfos?.find(t => t.tokenAccount === destinationATA && t.tokenMint === parsed.accounts.mint.address);
  ...
  async run() {
    ...
    const {
      message
    } = await cmd.typedCall('SolanaSignTx', 'SolanaTxSignature', this.params);
    return {
      signature: message.signature
    };
  }
