...
const {
  ...
  tokenProgramsInfo
} = blockchain_link_utils_1.solanaUtils;
...
const addPriorityFees = async (message, priorityFees) => {
  ...
  return (0, ts_belt_1.pipe)(message, m => prependTransactionMessageInstructions([getSetComputeUnitLimitInstruction({
...
const buildTransferTransaction = async (fromAddress, toAddress, amountInSol, blockhash, lastValidBlockHeight, priorityFees) => {
  ...
  const message = await (0, ts_belt_1.pipe)(createTransactionMessage({
    version: 'legacy'
  }), m => setTransactionMessageFeePayer(address(fromAddress), m), m => setTransactionMessageLifetimeUsingBlockhash({
    blockhash: blockhash,
...
const buildTokenTransferTransaction = async (fromAddress, toAddress, toAddressOwner, tokenMint, tokenUiAmount, tokenDecimals, fromTokenAccounts, toTokenAccount, blockhash, lastValidBlockHeight, priorityFees, tokenProgramName) => {
  ...
  let message = await (0, ts_belt_1.pipe)(createTransactionMessage({
    version: 'legacy'
  }), m => setTransactionMessageFeePayer(address(fromAddress), m), m => setTransactionMessageLifetimeUsingBlockhash({
    blockhash: blockhash,
    lastValidBlockHeight: BigInt(lastValidBlockHeight ?? '0xFFFFFFFFFFFFFFFF')
...
const fetchAccountOwnerAndTokenInfoForAddress = async (blockchain, address, mint, tokenProgram) => {
  ...
  if (accountInfoResponse) {
    ...
    tokenInfo = accountInfoResponse?.tokens?.find(token => token.contract === mint)?.accounts?.find(account => associatedTokenAccount.toString() === account.publicKey);
  }
