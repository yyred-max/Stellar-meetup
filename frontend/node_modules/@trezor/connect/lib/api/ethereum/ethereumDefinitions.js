...
const getEthereumDefinitions = async ({
  chainId,
  slip44,
  contractAddress
}) => {
...
const decodeEthereumDefinition = encodedDefinition => {
  ...
  ['encoded_token', 'encoded_network'].forEach(key => {
    ...
    const {
      definitionType,
      protobufPayload
    } = protocol_1.trzd.decode(encodedPayload);
    const {
      message: decodedDefinition
    } = (0, protobuf_1.decodeMessage)(proto, definitionType === 0 ? 'EthereumNetworkInfo' : 'EthereumTokenInfo', protobufPayload);
