...
class EthereumSignTransaction extends AbstractMethod_1.AbstractMethod {
  ...
  async payloadToPrecomposed() {
    try {
      ...
      return {
        ...
        maxFeePerGas: this.payload.transaction.maxFeePerGas ? new utils_1.BigNumber(this.payload.transaction.maxFeePerGas).dividedBy(1e9).toString() : undefined,
        maxPriorityFeePerGas: this.payload.transaction.maxPriorityFeePerGas ? new utils_1.BigNumber(this.payload.transaction.maxPriorityFeePerGas).dividedBy(1e9).toString() : undefined,
  ...
  async run() {
    ...
    const signature = isLegacy ? await helper.ethereumSignTx(this.device.getCommands().typedCall, this.params.path, tx.to, tx.value, tx.gasLimit, tx.gasPrice, tx.nonce, tx.chainId, chunkify, tx.data, tx.txType, definitions, tx.payment_req) : await helper.ethereumSignTxEIP1559(this.device.getCommands().typedCall, this.params.path, tx.to, tx.value, tx.gasLimit, tx.maxFeePerGas, tx.maxPriorityFeePerGas, tx.nonce, tx.chainId, chunkify, tx.data, tx.accessList, definitions, tx.payment_req);
