...
class TezosGetPublicKey extends AbstractMethod_1.AbstractMethod {
  ...
  get confirmation() {
    return {
      view: 'export-address',
      label: this.params.length > 1 ? 'Export multiple Tezos public keys' : `Export Tezos public key for account #${(0, pathUtils_1.fromHardened)(this.params[0].address_n[2]) + 1}`
    };
  }
  async run() {
    const responses = [];
